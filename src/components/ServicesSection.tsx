    ...
    <section id="services" className="bg-primary-100 py-section">
      <div className="container-content">
        <div className="max-w-2xl">
          ...
          <Reveal delay={150}>
            <p className="mt-5 text-base leading-relaxed text-secondary/60 md:text-lg">
              From first consultation to final handover, we deliver every
              discipline under one roof — unified by a single design language.
            </p>
          </Reveal>
        </div>

        <div className="mt-14 grid grid-cols-1 gap-px  sm:grid-cols-2 lg:grid-cols-3">
          {services.map((service, i) => (
            <Reveal key={service.id} delay={(i % 3) * 80}>
              <Link
                href={`/services/${service.slug}`}
                className="group relative block h-full bg-primary-100 p-7 transition-colors duration-500 hover:bg-secondary md:p-8"
              >
                {/* Number */}
                <div className="font-mono text-sm text-secondary/30 transition-colors duration-500 group-hover:text-primary/30">
                  {String(i + 1).padStart(2, '0')}
